...
def adminlogin():
	...
	if request.method == 'POST':
		...
		#if uname and pw does equal specified value then redirect to Admin opt. page
		else: 
			return redirect(url_for('admin'))
	#renders adminlogin forms
	return render_template('adminlogin.html', error=error)
	
#admin opt page with 3 functions 
...
#Form for adding a new linux user 
@app.route("/linuxuser")
def my_form():
...
def my_form_post():
	...
	return str(subprocess.check_call(["./addlinuxuser.sh", x,passwd], shell=False)) and redirect(url_for('new'))


#Remove linux user 
@app.route('/removelinuxuser', methods=['GET'])
def remove_form():
	return render_template('removelinuxuser.html')
...
def remove_form_post():
	...
	x = username
	passwd = request.form['pass']
	return str(subprocess.check_call(["./removelinuxuser.sh", x,passwd], shell=False))



#Admin add new user function/ to pos image repo 
@app.route("/capture", methods =['GET','POST'])
def admincapture():	
	if request.method == 'GET':
		#specific imports <--see if I can move these to the top of file --->
		#import glob
		#import os
		#import sys
		#import select
...
def per_scripts():
	...
	lone = request.form['lone']
	ltwo = request.form['ltwo']
	lthree = request.form['lthree']
	return 'username' 
	#return str(subprocess.check_call(["./removelinuxuser.sh", x,passwd], shell=False))
...
def user_login():
	return str(subprocess.check_call(["./userlogin.sh"], shell=False))
